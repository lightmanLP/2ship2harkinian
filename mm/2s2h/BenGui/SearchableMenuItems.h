...
static std::map<DisableOption, disabledInfo> disabledMap = {
    ...
    { DISABLE_FOR_MOUSE_ON,
      { [](disabledInfo& info) -> bool { return CVarGetInteger("gEnhancements.Camera.Mouse.Enable", 0); },
        "Mouse is Enabled" } },
    { DISABLE_FOR_MOUSE_OFF,
      { [](disabledInfo& info) -> bool { return !CVarGetInteger("gEnhancements.Camera.Mouse.Enable", 0); },
        "Mouse is Disabled" } },
          ...
          { {
            ...
            { { .widgetName = "Cameras", .widgetType = WIDGET_SEPARATOR_TEXT },
              ...
              { "Invert Mouse Camera Y Axis",
                ...
                nullptr,
                [](widgetInfo& info) { info.isHidden = disabledMap.at(DISABLE_FOR_MOUSE_OFF).active; } },
              { "Third-Person Mouse Horizontal Sensitivity: %.0f",
                "gEnhancements.Camera.Mouse.CameraSensitivity.X",
                "",
                WIDGET_CVAR_SLIDER_FLOAT,
                ...
                nullptr,
                [](widgetInfo& info) { info.isHidden = disabledMap.at(DISABLE_FOR_MOUSE_OFF).active; } },
              { "Third-Person Mouse Vertical Sensitivity: %.0f",
                "gEnhancements.Camera.Mouse.CameraSensitivity.Y",
                "",
                WIDGET_CVAR_SLIDER_FLOAT,
                // TODO: check boundaries
                { .min = 0.01f, .max = 5.0f, .defaultVariant = 1.0f, .format = "%.0f%%", .isPercentage = true },
                nullptr,
                [](widgetInfo& info) { info.isHidden = disabledMap.at(DISABLE_FOR_MOUSE_OFF).active; } },
              { "First-Person Mouse invert X Axis",
                ...
                nullptr,
                [](widgetInfo& info) { info.isHidden = disabledMap.at(DISABLE_FOR_MOUSE_OFF).active; } },
              { "First-Person Mouse invert Y Axis",
                ...
                nullptr,
                [](widgetInfo& info) { info.isHidden = disabledMap.at(DISABLE_FOR_MOUSE_OFF).active; } },
              { "First-Person Mouse Horizontal Sensitivity: %.0f",
                "gEnhancements.Camera.Mouse.FirstPerson.SensitivityX",
                "",
                WIDGET_CVAR_SLIDER_FLOAT,
                // TODO: check boundaries
                { .min = 0.01f, .max = 5.0f, .defaultVariant = 1.0f, .format = "%.0f%%", .isPercentage = true },
                nullptr,
                [](widgetInfo& info) { info.isHidden = disabledMap.at(DISABLE_FOR_MOUSE_OFF).active; } },
              { "First-Person Mouse Vertical Sensitivity: %.0f",
                "gEnhancements.Camera.Mouse.FirstPerson.SensitivityY",
                "",
                WIDGET_CVAR_SLIDER_FLOAT,
                // TODO: check boundaries
                { .min = 0.01f, .max = 5.0f, .defaultVariant = 1.0f, .format = "%.0f%%", .isPercentage = true },
